...
* The token is valid for use within its 10 mins life period, even if a new token is generated before the old one is expired. So, if in any chance the user requested a translation while the token is generated and before saving it into firebase, the translate request will be sent with the old token which is still valid(because only 6 minuets passed from its creation, still has more 4 minutes).
*/
...
/**
  ...
  * @param 'text' source text to be translated
           ...
           'format' text format
  */
  /*translateGoogle: function(text, target, callback, errorback) {
